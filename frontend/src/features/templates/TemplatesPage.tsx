...
import { Alert, Box, Button, Card, CardContent, Chip, Stack, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, TextField } from '@mui/material';
...
export function TemplatesPage() {
  ...
  return (
    <Stack spacing={2}>
      <PageHeader title="Plantillas" subtitle="Catalogo operativo de documentos pdfme." />
      {error ? <Alert severity="error">{error}</Alert> : null}
      {can(user, 'templates.create') ? <Card><CardContent><Stack component="form" direction={{ xs: 'column', md: 'row' }} spacing={1.5} onSubmit={create}><TextField label="Nombre" onChange={(event) => setName(event.target.value)} value={name} /><TextField label="Descripcion" onChange={(event) => setDescription(event.target.value)} value={description} /><TextField label="Etiquetas" onChange={(event) => setTags(event.target.value)} value={tags} /><Button startIcon={<PlusOutlined />} type="submit" variant="contained">Crear</Button></Stack></CardContent></Card> : null}
